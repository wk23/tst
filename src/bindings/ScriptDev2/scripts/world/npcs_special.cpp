...
struct MANGOS_DLL_DECL npc_engAI : public ScriptedAI
{
    npc_engAI(Creature *c) : ScriptedAI(c) {Reset();}
        ...
        }else 
{
        ...
        {                                                   
            ...
            case 2675:
 //error_log("SD2: rand11 %d.", 135+rand()%31);
                if(m_creature->getVictim())
                {
                    ...
                    {
                    m_creature->CastSpell(m_creature,2457, true);
                    m_creature->CastSpell(m_creature->getVictim(),7922, true);
                    m_creature->getVictim()->CastSpell(m_creature->getVictim(), 43418, true);
                    //DoCast(m_creature,43418);
                    //DoCast(m_creature,46222);
...
CreatureAI* GetAI_npc_eng(Creature *_Creature)
{
    return new npc_engAI (_Creature);
}
...
void AddSC_npcs_special()
{
    ...
    newscript->Name = "npc_eng";
    newscript->GetAI = &GetAI_npc_eng;
