...
struct MANGOS_DLL_DECL boss_shahrazAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        CurrentSaberLash = 0;
        CurrentBeam = rand()%3;                                    // 0 - Sinister, 1 - Vile, 2 - Wicked, 3 - Sinful
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (SaberLashTimer < diff)
        {
            ...
            if (!target || !target->isAlive())
                return;
            if (CurrentSaberLash == 51)
               {DoCast(target, SPELL_SABER_LASH);
                DoCast(target, SPELL_SABER_LASH_IMM);}
            SaberLashTimer = 30000;
            CurrentSaberLash = rand()%100;
