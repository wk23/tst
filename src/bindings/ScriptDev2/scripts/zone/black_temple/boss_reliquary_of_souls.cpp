...
void UpdateAI(const uint32 diff)
{
  ...
  if (Phase == 2)
  {
    if (SufferingGUID)
    {
      Creature* EssenceSuffering = ((Creature*)Unit::GetUnit((*m_creature), SufferingGUID));
      ...
      if (DespawnEssenceTimer < diff && !EssenceSuffering->isAlive())
      {
  ...
  if (Phase == 4)
  {
    if (DesireGUID)
    {
      ...
      EnterEvadeMode();
    if (DespawnEssenceTimer < diff  && !EssenceDesire->isAlive())
    {
...
struct MANGOS_DLL_DECL boss_essence_of_desireAI : public ScriptedAI
{
    ...
    void DamageTaken(Unit *done_by, uint32 &damage)
    {
        /*if ((damage >= m_creature->GetHealth()))
        {
            if (StatAuraGUID)
            {
                Unit* pUnit = SelectUnit(SELECT_TARGET_ANY_ENEMY);
                if (pUnit)
                    pUnit->RemoveAurasDueToSpell(AURA_OF_DESIRE);
            }
        }*/
    ...
    {
        DoZoneInCombat();
        who->RemoveAurasDueToSpell(AURA_OF_SUFFERING_ARMOR);
        DoCast(who, AURA_OF_DESIRE, true);
    }
...
struct MANGOS_DLL_DECL boss_essence_of_angerAI : public ScriptedAI
{
    ...
    {
        DoZoneInCombat();
        who->RemoveAurasDueToSpell( AURA_OF_DESIRE);
    ...
    void MoveInLineOfSight(Unit *who)
    {
        ...
        if (who->isTargetableForAttack() && who->isInAccessablePlaceFor(m_creature))
        {
            float attackRadius = m_creature->GetAttackDistance(who);
            if (m_creature->IsWithinDistInMap(who, attackRadius))
            {
...
{
    DoCast(m_creature, SPELL_SOUL_RELEASE);
    if (ReliquaryGUID)
    {
