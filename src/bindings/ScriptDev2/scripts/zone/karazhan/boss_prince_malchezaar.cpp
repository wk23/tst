...
struct MANGOS_DLL_DECL boss_malchezaarAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        InfernalCleanup();
        positions.clear();
         if (GameObject* mDoor = pInstance->instance->GetGameObject(pInstance->GetData64(DATA_GAMEOBJECT_NETHER_DOOR)))
            mDoor->SetGoState(GO_STATE_ACTIVE);
        for(int i =0; i < 5; ++i)
        {
    ...
    void Aggro(Unit *who)
    {
        ...
        if (GameObject* mDoor = pInstance->instance->GetGameObject(pInstance->GetData64(DATA_GAMEOBJECT_NETHER_DOOR)))
            mDoor->SetGoState(GO_STATE_READY);
    }
