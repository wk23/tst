    ...
    void OpenDoor(uint64 DoorGUID, bool open)
     {
        ...
        if(Player* i_pl = i->getSource())
            if(GameObject *Door = instance->GetGameObject(((ScriptedInstance*)instance)->GetData64(DoorGUID)))
                Door->SetUInt32Value(GAMEOBJECT_STATE, open ? GO_STATE_ACTIVE : GO_STATE_READY); 
    }
