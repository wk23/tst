...
void BattleGroundAV::Update(uint32 diff)
{
    ...
    if (GetStatus() == STATUS_WAIT_JOIN && GetPlayersSize())
    {
        ...
        for(BattleGroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
            ...
            ((plr->m_movementInfo.x>-1800) && (plr->m_movementInfo.x<1200)) && 
            ((plr->m_movementInfo.x != plr->GetBattleGroundEntryPoint().coord_x && plr->GetTeam() == ALLIANCE && plr->m_movementInfo.x<770.0f) || (plr->m_movementInfo.x != plr->GetBattleGroundEntryPoint().coord_x && plr->GetTeam() == HORDE && plr->m_movementInfo.x>-1360.0f)))
            {
                sLog.outBan("Player %s (GUID: %u) banned on AV - exit before opening of doors x:[%f] y:[%f] z:[%f] m[%u] e_x:[%f] e_y:[%f] e_z:[%f] e_m[%u]",plr->GetName(),plr->GetGUIDLow(),plr->m_movementInfo.x,plr->m_movementInfo.y,plr->m_movementInfo.z,plr->GetMapId(),plr->GetBattleGroundEntryPoint().coord_x,plr->GetBattleGroundEntryPoint().coord_y,plr->GetBattleGroundEntryPoint().coord_z,plr->GetBattleGroundEntryPoint().mapid);
