...
void BattleGroundAV::Update(uint32 diff)
{
    ...
    if (GetStatus() == STATUS_WAIT_JOIN && GetPlayersSize())
    {
        ...
        for(BattleGroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
            ...
            if (!plr->isGameMaster() && ((plr->m_movementInfo.x != plr->GetBattleGroundEntryPoint().coord_x && plr->GetTeam() == ALLIANCE && plr->m_movementInfo.x<770.0f) || (plr->m_movementInfo.x != plr->GetBattleGroundEntryPoint().coord_x && plr->GetTeam() == HORDE && plr->m_movementInfo.x>-1360.0f)))
            {
                sLog.outError("Player %s (GUID: %u) banned on AV - exit before opening of doors x:[%f] y:[%f] e_x:[%f] e_y:[%f]",plr->GetName(),plr->GetGUIDLow(),plr->m_movementInfo.x,plr->m_movementInfo.y,plr->GetBattleGroundEntryPoint().coord_x,plr->GetBattleGroundEntryPoint().coord_y);
                //std::string reason = "AV - exit before opening of doors by character ";
                //reason.append(plr->GetName());
                //sWorld.BanAccount(BAN_CHARACTER, plr->GetName(), "-1", reason,"AV_autoban");
            }
