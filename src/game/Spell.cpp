            ...
            case TARGET_CASTER_COORDINATES:
                switch(m_spellInfo->EffectImplicitTargetB[i])
                {
                    case TARGET_ALL_ENEMY_IN_AREA:
                        ...
                        if (m_spellInfo->Effect[i] == SPELL_EFFECT_ENVIRONMENTAL_DAMAGE || m_spellInfo->Id == 45246)
                        {
...
void Spell::SetTargetMap(uint32 effIndex,uint32 targetMode,UnitList& TagUnitMap)
{
    ...
    switch(targetMode)
    {
        ...
        case TARGET_CASTER_COORDINATES:
        {
            ...
            if (IS_GAMEOBJECT_GUID(m_originalCasterGUID) && m_caster->IsInWorld())
...
void Spell::SendSpellCooldown()
{
    ...
    if(m_spellInfo->Attributes & (SPELL_ATTR_DISABLED_WHILE_ACTIVE | SPELL_ATTR_PASSIVE | SPELL_ATTR_CANT_CANCEL))
        return;
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->GetTypeId()==TYPEID_PLAYER && !(m_spellInfo->Attributes & SPELL_ATTR_PASSIVE) && !(m_spellInfo->Attributes & SPELL_ATTR_CANT_CANCEL) &&
